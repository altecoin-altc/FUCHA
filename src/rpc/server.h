...
extern UniValue DoZfuchaSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzfuchaseed(const UniValue& params, bool fHelp);
extern UniValue getzfuchaseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzfucha(const UniValue& params, bool fHelp);
extern UniValue dzfuchastate(const UniValue& params, bool fHelp);
