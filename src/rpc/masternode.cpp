...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) fucha address of current masternode\n"
...
UniValue listmasternodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode fucha address\n"
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:18145\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:18145\""));
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) fucha address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) fucha MN Address\n"
